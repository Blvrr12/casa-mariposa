...
function SplashScreen() {
  const [show, setShow] = useState(true);        // visibilidad general
  const [exitNow, setExitNow] = useState(false); // activa salida animada

  useEffect(() => {
    const handleLoad = () => {
      setExitNow(true); // activa la salida (zoom)
      setTimeout(() => setShow(false), 1000); // da tiempo al fade+zoom
    };

    // si ya cargó al entrar
    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          animate={exitNow ? { opacity: 0, scale: 4 } : { opacity: 1, scale: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          >
            <motion.path
              d="M29.238,4.27c-0.48-0.013-0.956,0.036-1.411,0.132c-0.456,0.095-0.896,0.228-1.308,0.399
